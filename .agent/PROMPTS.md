# PROMPTS.md — AutoResearcher Root
> Reusable LLM prompt templates for all agents.
> All prompts are versioned. Update the version and log changes when modifying.

---

## How to Use This File

- Prompts are stored here as source-of-truth templates
- Import and use them from `app/core/prompts.py` in the backend
- When a prompt is changed, increment the version and note what changed
- Test prompt changes against at least 3 real inputs before committing

---

## Planner Agent

### `PLANNER_DECOMPOSE_QUERY` v1.0
...
### `ANALYSIS_SUMMARIZE_PAPER` v1.0

```
You are an expert research analyst. Analyze the following research paper
...
### `ANALYSIS_COMPARE_PAPERS` v1.0

```
You are a research analyst comparing multiple academic papers.
...
### `INSIGHT_FIND_GAPS` v1.0

```
You are a senior research scientist identifying research gaps.
...
### `WRITER_LITERATURE_REVIEW` v1.0

```
You are an academic writing assistant producing a literature review.
...
### `WRITER_PAPER_SUMMARY_SHORT` v1.0

```
Summarize the following research paper in 3 sentences for a general
...
### `RETRIEVAL_ANSWER_QUESTION` v1.0

```
You are a research assistant answering questions using retrieved paper excerpts.
...
- Always request JSON output when structured data is needed
- Include exact field names in the prompt to enforce schema
- Add "Respond with ONLY the JSON" to prevent preamble
- For long papers, truncate content to fit context window before passing
- Test prompts with edge cases: empty abstracts, non-English papers, very short papers
